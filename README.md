...
将常用设计系统保存为品牌模板（`themes/{品牌名}/`），下次使用同一品牌时直接调用，保持视觉一致性，跳过重复的设计系统声明步骤。
...
- **主题市场**：预置多套开箱即用的品牌主题（Apple / Linear / Stripe / Notion 风格）
- **资产库充实**：内置常用背景图、图标集、插画素材
- **响应式检查**：自检阶段增加移动端断点验证
- **无障碍检查**：自动检测对比度、ARIA 标签等 A11y 问题
